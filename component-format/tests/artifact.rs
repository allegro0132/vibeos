...
use vibeos_component_format::{
    ...
    PROFILE_7_SYNC_REFERENCE_TYPES_EXECUTABLE_PROFILE_CODE,
};
...
#[test]
fn c810_code7_roundtrips_but_remains_validation_only() {
    let artifact = artifact(ProfileIdentity::PROFILE_4_SYNC_SIMD_VALIDATION);
    let encoded = artifact.encode().unwrap();
    assert_eq!(
    ...
    assert_eq!(read_u16(&encoded, PROFILE_STAGE_OFFSET), 2);
    assert_eq!(read_u16(&encoded, ARTIFACT_ABI_OFFSET), 7);
    assert_eq!(read_u16(&encoded, COMPONENT_PROFILE_OFFSET), 4);
    assert_eq!(read_u16(&encoded, CORE_PROFILE_OFFSET), 4);
    assert_eq!(read_u16(&encoded, RUNTIME_ABI_OFFSET), 7);
    let decoded = ComponentArtifactV1::decode(&encoded).unwrap();
    assert_eq!(
        decoded.profile(),
...
#[test]
fn c811_code8_roundtrips_without_reinterpreting_code7() {
    let artifact = artifact(ProfileIdentity::PROFILE_5_SYNC_SIMD_EXECUTABLE);
    let encoded = artifact.encode().unwrap();
    assert_eq!(
        read_u16(&encoded, PROFILE_CODE_OFFSET),
        PROFILE_5_SYNC_SIMD_EXECUTABLE_PROFILE_CODE
    );
    assert_eq!(read_u16(&encoded, PROFILE_STAGE_OFFSET), 1);
    assert_eq!(read_u16(&encoded, ARTIFACT_ABI_OFFSET), 8);
    assert_eq!(read_u16(&encoded, COMPONENT_PROFILE_OFFSET), 5);
    assert_eq!(read_u16(&encoded, CORE_PROFILE_OFFSET), 5);
    assert_eq!(read_u16(&encoded, RUNTIME_ABI_OFFSET), 8);
    let decoded = ComponentArtifactV1::decode(&encoded).unwrap();
    assert_eq!(
        decoded.profile(),
...
#[test]
fn c813_code10_roundtrips_without_reinterpreting_code9() {
    let artifact = artifact(ProfileIdentity::PROFILE_7_SYNC_REFERENCE_TYPES_EXECUTABLE);
    let encoded = artifact.encode().unwrap();
    assert_eq!(
        read_u16(&encoded, PROFILE_CODE_OFFSET),
        PROFILE_7_SYNC_REFERENCE_TYPES_EXECUTABLE_PROFILE_CODE
    ...
    assert_eq!(read_u16(&encoded, PROFILE_STAGE_OFFSET), 1);
    assert_eq!(read_u16(&encoded, ARTIFACT_ABI_OFFSET), 10);
    assert_eq!(read_u16(&encoded, COMPONENT_PROFILE_OFFSET), 7);
    assert_eq!(read_u16(&encoded, CORE_PROFILE_OFFSET), 7);
    assert_eq!(read_u16(&encoded, RUNTIME_ABI_OFFSET), 10);
    let decoded = ComponentArtifactV1::decode(&encoded).unwrap();
    assert_eq!(
        decoded.profile(),
