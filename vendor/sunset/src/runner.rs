...
pub struct Runner<'a, CS: conn::CliServ> {
    ...
    /// Caller-provided random source, exclusively borrowed by this connection.
...
impl<'a> Runner<'a, client::Client> {
    ...
    pub fn term_break(&mut self, chan: &ChanHandle, length: u32) -> Result<()> {
        let mut s = self.traf_out.sender(&mut self.keys, self.random);
    ...
    pub(crate) fn cli_session_opener(
        ...
        let s = self.traf_out.sender(&mut self.keys, self.random);
...
impl<'a> Runner<'a, client::Client> {
    ...
    pub fn new_client_owned(random: &'a mut dyn RandomSource) -> Self {
        Self::new_owned(random)
    }
}
...
impl<'a> Runner<'a, server::Server> {
    ...
    pub(crate) fn resume_servhostkey_ed25519(
        ...
        let (payload, _seq) = self.traf_in.payload().trap()?;
        let mut s = self.traf_out.sender(&mut self.keys, self.random);
        self.conn.resume_servhostkey_ed25519(payload, &mut s, signer)?;
        self.resume(&DispatchEvent::ServEvent(ServEventId::Hostkeys));
        Ok(())
    }
    ...
    /// Send the exit status for an accepted exec request.
    ///
    /// The strict server profile requires this after all stdout/stderr data and
    /// before EOF. It may be queued internally while the output buffer drains.
    ...
    /// Send EOF after the exit status for an accepted exec request.
    ...
    /// Close a completed exec channel after its EOF has been sent.
    pub fn close_channel(&mut self, chan: &ChanHandle) -> Result<()> {
        let mut s = self.traf_out.sender(&mut self.keys, self.random);
        self.conn.channels.send_close(chan.0, &mut s)?;
        self.wake();
        Ok(())
    }
}
...
impl<'a> Runner<'a, server::Server> {
    ...
    ///
    /// Only available running with `alloc` or `std` feature.
    pub fn new_server_owned(random: &'a mut dyn RandomSource) -> Self {
        Self::new_owned(random)
    }
}
...
impl<'a, CS: CliServ> Runner<'a, CS> {
    pub fn new(
        ...
        Self::new_traf(TrafIn::new(inbuf), TrafOut::new(outbuf), random)
    }
    ...
    pub fn progress(&mut self) -> Result<Event<'_, 'a>> {
        ...
        if !self.conn.is_kex_sending() {
            match self.traf_out.send_deferred_packets(&mut self.keys, self.random) {
                Ok(()) => (),
                Err(Error::NoRoom { .. }) => {
        ...
        if let Some((payload, seq)) = self.traf_in.payload() {
            ...
            // A peer disconnect is terminal. Consume the packet exactly once,
            // discard any pending output, and make subsequent I/O observe EOF.
            if disp.disconnect {
                self.traf_in.done_payload();
                self.closed_input = true;
                self.traf_out.close();
                self.wake();
                return if CS::is_client() {
                    Ok(Event::Cli(CliEvent::Defunct))
                } else {
...
#[cfg(feature = "alloc")]
impl<'a, CS: CliServ> Runner<'a, CS> {
    pub fn new_owned(random: &'a mut dyn RandomSource) -> Runner<'a, CS> {
        Self::new_traf(TrafIn::new_owned(), TrafOut::new_owned(), random)
    }
}
...
mod tests {
    ...
    fn initial_output(seed: u8) -> std::vec::Vec<u8> {
        let mut input = std::vec![0; 4096];
        let mut output = std::vec![0; 4096];
        let mut random = TestRandom::new(seed);
        let mut runner = Runner::<server::Server>::new_server(
            &mut input,
            &mut output,
            &mut random,
        );

        assert!(matches!(runner.progress().unwrap(), Event::Progressed));
        let version_len = runner.output_buf().len();
        runner.consume_output(version_len);

        assert!(matches!(runner.progress().unwrap(), Event::None));
        runner.output_buf().to_vec()
    }

    #[test]
    fn runner_randomness_is_caller_deterministic() {
        assert_eq!(initial_output(0x61), initial_output(0x61));
        assert_ne!(initial_output(0x61), initial_output(0x62));
    }

    struct FailOnceRandom {
        failed: bool,
    }

    impl RandomSource for FailOnceRandom {
    ...
    #[test]
    fn random_failure_precedes_output_and_state_advance() {
        let mut input = std::vec![0; 4096];
        let mut output = std::vec![0; 4096];
        let mut random = FailOnceRandom { failed: false };
        let mut runner = Runner::<server::Server>::new_server(
            &mut input,
            &mut output,
            &mut random,
        ...
        assert!(matches!(runner.progress(), Err(Error::Random)));
        assert!(!runner.is_output_pending());

        // A retry succeeds, proving the failed call left KEX at its retryable
        // pre-cookie state rather than advancing it halfway.
        assert!(matches!(runner.progress().unwrap(), Event::None));
        assert!(runner.is_output_pending());
    }
}
