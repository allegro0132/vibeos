#!/usr/bin/env python3
"""Fail-closed C8.8-F2 RISC-V software-float object audit.
...
uses Cargo JSON package IDs to separate the reviewed fork from stock Wasmi,
and scans both LLVM IR and lowered ``riscv64imac`` assembly.
...
EXPECTED_ARTIFACTS = {
    "vibeos-wasm-float-candidate": ROOT / "wasm-float-candidate/Cargo.toml",
    "vibeos-wasmi-softfloat": ROOT
    / "vendor/wasmi-softfloat/crates/wasmi/Cargo.toml",
    "vibeos-wasmi-core-softfloat": ROOT
    / "vendor/wasmi-softfloat/crates/core/Cargo.toml",
    "vibeos-wasmi-ir-softfloat": ROOT
    / "vendor/wasmi-softfloat/crates/ir/Cargo.toml",
    "vibeos-wasmi-collections-softfloat": ROOT
...
def audit_detector_patterns() -> None:
    ...
    for fixture in opcode_fixtures:
        if RISCV_FP_OPCODE_RE.search(fixture) is None:
            fail(f"RISC-V F/D detector missed its self-test fixture: {fixture}")
...
def audit_static_manifests() -> None:
    ...
    candidate = load_toml(ROOT / "wasm-float-candidate/Cargo.toml")
    ...
    core = load_toml(ROOT / "vendor/wasmi-softfloat/crates/core/Cargo.toml")
    ...
    if core_dependencies.get("libm") != expected_libm:
        fail(f"fork optional libm edge drifted: {core_dependencies.get('libm')!r}")
    if core.get("features", {}).get("simd") != ["dep:libm"]:
        fail("fork core SIMD feature must remain the sole gate for optional libm")
...
def audit_resolved_graph(
    ...
    candidate = unique_package(packages, "vibeos-wasm-float-candidate", "0.1.0")
    ...
    for name, expected_features in EXPECTED_FORK_FEATURES.items():
        ...
        if "simd" in actual or "std" in actual:
            fail(f"forbidden fork feature enabled for {name}: {sorted(actual)}")

    fork_core = selected["vibeos-wasmi-core-softfloat"]
    fork_core_deps = {
    ...
    stock_packages: dict[str, Package] = {}
    for name in STOCK_CHECKSUMS:
        package = unique_package(packages, name, "1.1.0")
        if package.source != REGISTRY_SOURCE or package.package_id not in reachable:
            fail(f"Profile-1 stock dependency is not independently resolved: {name}")
        if package.package_id in {item.package_id for item in selected.values()}:
            fail(f"stock and candidate package identity collided: {name}")
        stock_packages[name] = package

    libm = unique_package(packages, "libm", "0.2.16")
    reverse_parents: set[str] = set()
    for parent_id in reachable:
        parent = nodes[parent_id]
        if libm.package_id in set(non_dev_dependencies(parent)):
            reverse_parents.add(parent_id)
    expected_parent = stock_packages["wasmi_core"].package_id
    if reverse_parents != {expected_parent}:
        parent_names = sorted(packages[item].name for item in reverse_parents)
        fail(f"libm escaped the stock Profile-1 closure: parents={parent_names}")
...
def cargo_build_artifacts(
    ...
    for number, line in enumerate(result.stdout.splitlines(), 1):
        ...
        if package_id in stock_ids and filenames:
            name = stock_ids[package_id]
            if len(filenames) != 1 or name in stock_rlibs:
                fail(f"ambiguous stock rlib artifact for {name}: {filenames}")
            stock_rlibs[name] = filenames[0]

    missing = sorted(set(EXPECTED_ARTIFACTS) - set(candidate_rlibs))
    if missing:
        fail(f"Cargo JSON omitted candidate rlibs: {missing}")
    missing_stock = sorted(set(STOCK_CHECKSUMS) - set(stock_rlibs))
    if missing_stock:
        fail(f"Cargo JSON omitted independently resolved stock rlibs: {missing_stock}")

    target_root = (target_dir / TARGET / "release").resolve()
    for name, path in {**candidate_rlibs, **stock_rlibs}.items():
        if not path.is_file() or not path.is_relative_to(target_root):
            fail(f"untrusted or missing Cargo artifact for {name}: {path}")
    if set(candidate_rlibs.values()) & set(stock_rlibs.values()):
        fail("candidate and stock Cargo artifacts overlap")
    return candidate_rlibs, stock_rlibs
...
def audit_rlib(
    ...
    for index, member in enumerate(object_members):
        ...
        fp_opcodes = matching_lines(asm_path, RISCV_FP_OPCODE_RE)
        ...
        if fp_opcodes:
            failures.append(
                describe_matches(package, member, "RISC-V F/D opcode", fp_opcodes)
            )
        if asm_helpers:
            failures.append(
...
def main() -> int:
    try:
        ...
        with tempfile.TemporaryDirectory(
            prefix="vibeos-c88-f2-riscv-object-"
        ) as temporary:
            ...
            candidate_rlibs, stock_rlibs = cargo_build_artifacts(
        ...
        print("C8.8-F2 RISC-V object audit: PASS")
        print(
            "toolchain: "
            f"{TOOLCHAIN} rustc={EXPECTED_RUSTC_COMMIT[:12]} "
            f"cargo={EXPECTED_CARGO_COMMIT[:12]} LLVM={EXPECTED_LLVM_VERSION}"
        )
        print(f"target: {TARGET}; features={','.join(sorted(target_features))}; f/d=absent")
        for report in reports:
            ...
            print(
                ...
                f"semantic-fp=0; fp-helpers=0; f/d-opcodes=0; "
                f"sign-only={sign_summary}; asm-proof=integer-only"
            )
        print(
            "stock isolation: excluded Cargo package IDs="
            + ",".join(sorted(stock_rlibs))
            + "; fork libm/simd=absent; stock checksums=verified"
        )
        return 0
    except AuditFailure as error:
        print(f"C8.8-F2 RISC-V object audit: FAIL\n{error}", file=sys.stderr)
        return 1
