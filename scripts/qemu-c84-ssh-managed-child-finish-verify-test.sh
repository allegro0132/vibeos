#!/bin/sh
# C8.4 real SSH finish/independent-verify/zero-cursor-discard gate.
set -eu
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$RESULT_REPORTED" -eq 0 ]; then
    echo 'FAIL qemu-c84-ssh-managed-child-finish-verify-test: test aborted unexpectedly' >&2
  fi
  exit "$status"
}
...
fail() {
  RESULT_REPORTED=1
  echo "FAIL qemu-c84-ssh-managed-child-finish-verify-test: $*" >&2
...
echo 'qemu-c84-ssh-managed-child-finish-verify-test: building isolated single-hart OpenSSH image'
...
if ! python3 -B scripts/c84-ssh-managed-child-finish-verify-peer.py \
  --host localhost \
  --port "$SSH_HOST_PORT" \
  ...
  >"$PEER_LOG" 2>&1; then
  fail 'real OpenSSH finish/verify sequence failed'
fi

kill -0 "$QEMU_PID" 2>/dev/null || fail 'QEMU exited during the OpenSSH sequence'
finish_count_before=$(family_count "$FAMILY")
[ "$finish_count_before" -eq 4 ] \
  || fail "closed peer transcript did not contain exactly 4 finish/verify markers: $finish_count_before"
...
sleep 0.2
finish_count_after=$(family_count "$FAMILY")
...
# Stop the UART producer before the final exact parse. This prevents an
# incomplete late record from being either hidden or spuriously admitted.
stop_qemu
finish_count_frozen=$(family_count "$FAMILY")
...
if grep -a -E -q "(${FAMILY}|${IRQ_FAMILY}|${PHASE_FAMILY}|${CORE_FAMILY}|${REQUEST_FAMILY}) (PASS|TOPOLOGY|SUMMARY|INTERVAL|COMPLETE|STREAM|PUBLISH|PUBLISHER|SCHEMA|COLLECTOR)([ =]|$)" "$QEMU_LOG"; then
...
foreign_families=$(grep -a -o -E 'WASM_[A-Z0-9_]+' "$QEMU_LOG" \
  | sort -u \
  | grep -E -v "^(${FAMILY}|${IRQ_FAMILY}|${PHASE_FAMILY}|${CORE_FAMILY}|${REQUEST_FAMILY})$" || true)
[ -z "$foreign_families" ] \
  || fail "diagnostic image emitted foreign WASM families: $foreign_families"

python3 -B scripts/c84-ssh-managed-child-finish-verify-peer.py \
  --verify-log-only --qemu-log "$QEMU_LOG" >&2 \
  || fail 'frozen finish/IRQ/phase/Core/request transcript verification failed'

RESULT_REPORTED=1
sed -n '1,8p' "$PEER_LOG"
echo 'PASS qemu-c84-ssh-managed-child-finish-verify-test: epochs 1/2/4 target finish plus independent verify, explicit zero-cursor StreamAbandoned discard, stored rejection/ack/Ready reuse, epoch-3 LeaseCancelled Drop, and exact IRQ/phase/19-Core/8-request predecessors passed on one QEMU hart (integration evidence only)'
