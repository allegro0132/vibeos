...
impl DurableRights {
    ...
    pub const fn from_bits(bits: u32) -> Option<Self> {
        if bits & !KNOWN_RIGHTS == 0 { Some(Self(bits)) } else { None }
    }
...
impl GrantFlags {
    ...
    const fn from_bits(bits: u32) -> Option<Self> {
        if bits & !Self::ROOT.0 == 0 { Some(Self(bits)) } else { None }
    }
...
pub enum RecordBody {
    Format,
    IdHighWater { exclusive_end: u128 },
    ...
    RevokeTombstone { derivation_id: DerivationId },
}
...
impl LogRecord {
    ...
    pub fn decode(bytes: &[u8; RECORD_SIZE]) -> Result<DecodeStatus, DecodeError> {
        ...
        let kind = RecordKind::from_raw(get_u16(bytes, 0x0a))
        ...
        Ok(DecodeStatus::Valid(DecodedRecord { record, crc32c: crc }))
    }
}
...
fn validate_envelope(record: &LogRecord) -> Result<(), EncodeError> {
    ...
    if record.sequence == 1
        && (record.previous_sequence != 0 || record.previous_crc32c != 0)
    {
    ...
    if matches!(
        record.body,
        RecordBody::GrantCommit { prepare_sequence: 0, .. }
    ) {
...
fn decode_body(bytes: &[u8; RECORD_SIZE], kind: RecordKind) -> Result<RecordBody, DecodeError> {
    Ok(match kind {
        ...
        RecordKind::GrantPrepare => {
            ...
            at += 4;
            let rights = DurableRights::from_bits(get_u32(bytes, at))
                .ok_or(DecodeError::UnknownRights)?;
            at += 4;
            let generation = get_u64(bytes, at);
            at += 8;
            let resource_kind = ResourceKind::new(get_u32(bytes, at))
                .ok_or(DecodeError::ZeroResourceKind)?;
            at += 4;
            let flags = GrantFlags::from_bits(get_u32(bytes, at))
                .ok_or(DecodeError::UnknownGrantFlags)?;
            RecordBody::GrantPrepare(GrantRecord {
                derivation_id,
                parent_id,
                object_id,
                target: SlotIdentity { space, slot, generation },
                rights,
                resource_kind,
                flags,
            })
...
impl RecordChain {
    pub const fn new(store_id: StoreId) -> Self {
        Self { store_id, next_sequence: 1, previous_sequence: 0, previous_crc32c: 0 }
    }
    ...
    pub fn append(
        ...
        self.next_sequence = self.next_sequence.checked_add(1).ok_or(EncodeError::ZeroSequence)?;
        Ok(bytes)
    }
}
...
pub fn recover(
    ...
    for (sector, bytes) in sectors.iter().enumerate() {
        match LogRecord::decode(bytes) {
            ...
            Ok(DecodeStatus::Valid(decoded)) => valid.push((sector, decoded)),
    ...
    if first.record.sequence != 1 || !matches!(first.record.body, RecordBody::Format) {
        return Err(RecoveryError::FormatNotFirst);
    }

    let mut previous_sequence = 0u64;
    let mut previous_crc = 0u32;
    for (index, (sector, decoded)) in valid.iter().enumerate() {
        let record = &decoded.record;
        if record.store_id != policy.store_id {
            return Err(RecoveryError::WrongStore { sector: *sector });
        }
        ...
        {
            return Err(RecoveryError::BrokenSequence { sector: *sector });
        }
        if index != 0 && matches!(record.body, RecordBody::Format) {
    ...
    for (_, decoded) in &valid {
        ...
        match &decoded.record.body {
            ...
            RecordBody::GrantPrepare(grant) => {
                let tx = decoded.record.transaction_id.expect("decoder requires transaction");
            ...
            RecordBody::GrantCommit {
                ...
                match transactions.get(&tx).cloned() {
                    Some(TxState::Prepared(prepared)) => {
    ...
    for recovered in &committed {
        ...
        if let Some(kind) = object_kinds.insert(grant.object_id, grant.resource_kind) {
            if kind != grant.resource_kind {
                return Err(RecoveryError::ObjectMismatch { sequence: recovered.commit_sequence });
            }
        }
        if grant.flags.is_root() {
            if grant.parent_id.is_some() {
                return Err(RecoveryError::RootShape { sequence: recovered.commit_sequence });
            }
            if !policy.roots.iter().any(|root| root.grant == *grant) {
                return Err(RecoveryError::RootNotTrusted { sequence: recovered.commit_sequence });
            }
        } else {
            ...
            let Some(parent) = graph.get(&parent_id) else {
                return Err(RecoveryError::MissingParent { sequence: recovered.commit_sequence });
            };
    ...
    for recovered in &committed {
        ...
        if let Some((old_generation, old_derivation)) = slots.get(&key).copied() {
            if old_generation == u64::MAX || recovered.grant.target.generation <= old_generation {
                return Err(RecoveryError::SlotGeneration { sequence: recovered.commit_sequence });
            }
            if !is_tombstoned_before(
                ...
                return Err(RecoveryError::SlotStillLive { sequence: recovered.commit_sequence });
            }
        }
        slots.insert(key, (recovered.grant.target.generation, recovered.grant.derivation_id));
    }
