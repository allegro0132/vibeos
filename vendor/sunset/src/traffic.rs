...
impl<'a> TrafOut<'a> {
    ...
    pub(crate) fn send_packet(
        ...
        if is_kex || (self.deferred_packets.is_empty() && !self.sending_kex) {
            ...
            match self.send_packet_inner(&p, keys, random) {
                Err(Error::NoRoom { .. }) => {
    ...
    pub fn send_packet_inner(
        ...
        // Encrypt in place
        let elen = keys.encrypt(plen, wbuf, random)?;
        self.state = TxState::Write { idx, len: len + elen };
        Ok(())
    }

    pub fn send_deferred_packets(
        &mut self,
        keys: &mut KeyState,
        random: &mut dyn RandomSource,
    ) -> Result<()> {
    ...
    pub fn sender<'s>(
        &'s mut self,
        keys: &'s mut KeyState,
        random: &'s mut dyn RandomSource,
    ) -> TrafSend<'s, 'a> {
        TrafSend::new(self, keys, random)
    }
...
impl<'s, 'a> TrafSend<'s, 'a> {
    fn new(
        ...
        Self { out, keys, random }
    }

    pub fn send<'p, P: Into<packets::Packet<'p>>>(&mut self, p: P) -> Result<()> {
        self.out.send_packet(p.into(), self.keys, self.random)
    }
...
pub enum DeferredPacket {
    ...
    /// Keep the lifetime-free fields needed to reconstruct the strict exec
    /// completion request after output backpressure clears.
    ChannelExitStatus {
        num: u32,
        status: u32,
    },
...
impl From<&DeferredPacket> for Packet<'static> {
    fn from(d: &DeferredPacket) -> Self {
        match d {
            ...
            DeferredPacket::ChannelExitStatus { num, status } => {
                packets::ChannelRequest {
                    num: *num,
                    want_reply: false,
                    req: packets::ChannelReqType::ExitStatus(packets::ExitStatus {
                        status: *status,
                    }),
                }
                .into()
    ...
    fn try_from(packet: Packet<'a>) -> Result<Self> {
        Ok(match packet {
            ...
            Packet::ChannelRequest(p)
                if !p.want_reply
                    && matches!(p.req, packets::ChannelReqType::ExitStatus(_)) =>
            {
                let packets::ChannelReqType::ExitStatus(status) = p.req else {
                    unreachable!()
                };
                Self::ChannelExitStatus { num: p.num, status: status.status }
            }
...
mod strict_exec_tests {
    ...
    fn exit_status_survives_deferred_output() {
        let packet = Packet::ChannelRequest(packets::ChannelRequest {
            num: 23,
            want_reply: false,
            req: packets::ChannelReqType::ExitStatus(packets::ExitStatus {
                status: 17,
        ...
        let deferred = DeferredPacket::try_from(packet).unwrap();

        assert!(matches!(
            Packet::from(&deferred),
            Packet::ChannelRequest(packets::ChannelRequest {
                num: 23,
                want_reply: false,
                req: packets::ChannelReqType::ExitStatus(packets::ExitStatus {
                    status: 17
    ...
    #[test]
    fn reply_seeking_exit_status_is_not_deferred() {
        let packet = Packet::ChannelRequest(packets::ChannelRequest {
            num: 0,
            want_reply: true,
            req: packets::ChannelReqType::ExitStatus(packets::ExitStatus {
