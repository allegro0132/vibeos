...
macro_rules! impl_sign_for {
    ( $(($ty:ty, $bits:ty, $sign:expr, $one:expr, $neg_one:expr)),* $(,)? ) => {
        $(
            impl From<$ty> for Sign<$ty> {
                fn from(value: $ty) -> Self {
                    Self::new(value.to_bits() & ($sign as $bits) == 0)
                }
            }
            ...
            impl From<Sign<$ty>> for $ty {
                fn from(sign: Sign<$ty>) -> Self {
                    match sign.is_positive {
                        true => <$ty>::from_bits($one),
                        false => <$ty>::from_bits($neg_one),
                    }
                }
            }
        )*
...
impl_sign_for!(
    (f32, u32, 0x8000_0000_u32, 0x3f80_0000_u32, 0xbf80_0000_u32),
    (
        f64,
        u64,
        0x8000_0000_0000_0000_u64,
        0x3ff0_0000_0000_0000_u64,
        0xbff0_0000_0000_0000_u64
    ),
);
