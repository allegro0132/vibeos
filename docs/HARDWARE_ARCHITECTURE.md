...
| `drivers/dwc2-host` | CV1800B USB clocks/role wiring, DWC2 reset/root-port power, buffer-DMA host channels, EP0 enumeration, HID boot-keyboard configuration and interrupt-IN reports | PLIC routing, storage classes and HID capability publication; the kernel adapter owns polling and hotplug recovery policy |
