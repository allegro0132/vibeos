macro_rules! float {
    (
        $( #[$docs:meta] )*
        struct $name:ident($prim:ty as $bits:ty, $eq:ident, $cmp:ident, $is_nan:ident);
    ) => {
        $(#[$docs])*
        #[derive(Copy, Clone)]
        ...
        impl ::core::cmp::PartialEq<$name> for $name {
            ...
            fn eq(&self, other: &$name) -> ::core::primitive::bool {
                crate::softfloat::$eq(self.to_bits(), other.to_bits())
            }
        }
        ...
        impl ::core::cmp::PartialOrd<$name> for $name {
            ...
            fn partial_cmp(&self, other: &$name) -> ::core::option::Option<::core::cmp::Ordering> {
                crate::softfloat::$cmp(self.to_bits(), other.to_bits())
            }
        }

        impl ::core::fmt::Debug for $name {
            fn fmt(&self, f: &mut ::core::fmt::Formatter) -> ::core::fmt::Result {
                if crate::softfloat::$is_nan(self.to_bits()) {
...
float! {
    /// A 32-bit `f32` type.
    struct F32(f32 as u32, f32_eq_bits, f32_partial_cmp_bits, f32_is_nan);
}

float! {
    /// A 64-bit `f64` type.
    struct F64(f64 as u64, f64_eq_bits, f64_partial_cmp_bits, f64_is_nan);
}
