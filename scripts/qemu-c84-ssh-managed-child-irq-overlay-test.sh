#!/bin/sh
# C8.4 real SSH parent/managed-child causal self-SSIP IRQ-overlay gate.
set -eu
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$RESULT_REPORTED" -eq 0 ]; then
    echo 'FAIL qemu-c84-ssh-managed-child-irq-overlay-test: test aborted unexpectedly' >&2
  fi
  exit "$status"
}
...
fail() {
  RESULT_REPORTED=1
  echo "FAIL qemu-c84-ssh-managed-child-irq-overlay-test: $*" >&2
  if [ -n "$PEER_LOG" ] && [ -s "$PEER_LOG" ]; then
    echo '--- OpenSSH driver output ---' >&2
    tail -100 "$PEER_LOG" >&2 || true
  fi
  if [ -n "$QEMU_LOG" ] && [ -s "$QEMU_LOG" ]; then
    echo '--- QEMU serial output (last 280 lines) ---' >&2
    tail -280 "$QEMU_LOG" >&2 || true
  fi
  exit 1
}
...
python3 -B scripts/verify-c84-ssh-managed-child-irq-overlay.py --selftest --check-source >&2 \
  || fail 'static managed-child IRQ-overlay verification failed'
...
python3 -B scripts/c84-ssh-managed-child-irq-overlay-peer.py --selftest >/dev/null \
  || fail 'IRQ-overlay marker parser self-test failed'
...
echo 'qemu-c84-ssh-managed-child-irq-overlay-test: building isolated single-hart OpenSSH image'
...
if ! python3 -B scripts/c84-ssh-managed-child-irq-overlay-peer.py \
  --host localhost \
  --port "$SSH_HOST_PORT" \
  ...
  >"$PEER_LOG" 2>&1; then
  fail 'real OpenSSH managed-child IRQ-overlay sequence failed'
fi

kill -0 "$QEMU_PID" 2>/dev/null || fail 'QEMU exited during the OpenSSH sequence'
irq_count_before=$(family_count "$FAMILY")
[ "$irq_count_before" -eq 6 ] \
  || fail "closed peer transcript did not contain exactly 6 IRQ-overlay markers: $irq_count_before"
phase_count_before=$(family_count "$PHASE_FAMILY")
case "$phase_count_before" in
  27|28) ;;
  *) fail "closed peer transcript did not preserve 27/28 exact phase markers: $phase_count_before" ;;
esac
...
[ "$request_count_before" -eq 8 ] \
  || fail "closed peer transcript did not preserve exactly 8 request-parent markers: $request_count_before"

sleep 0.2
irq_count_after=$(family_count "$FAMILY")
...
[ "$irq_count_after" -eq "$irq_count_before" ] \
  || fail "late IRQ marker arrived after peer closure: before=$irq_count_before after=$irq_count_after"
...
[ "$request_count_after" -eq "$request_count_before" ] \
  || fail "late request marker arrived after peer closure: before=$request_count_before after=$request_count_after"
...
stop_qemu
irq_count_frozen=$(family_count "$FAMILY")
...
[ "$irq_count_frozen" -eq "$irq_count_after" ] \
  || fail "IRQ marker arrived while freezing QEMU: before=$irq_count_after frozen=$irq_count_frozen"
...
[ "$request_count_frozen" -eq "$request_count_after" ] \
  || fail "request marker arrived while freezing QEMU: before=$request_count_after frozen=$request_count_frozen"
...
if grep -a -E -q "${PHASE_FAMILY} (FINISH|STREAM|PUBLISH|PUBLISHER|IRQ)([ =]|$)|${PHASE_FAMILY} .* (finish|stream|publisher|irq)=" "$QEMU_LOG"; then
  fail 'predecessor phase family emitted a forbidden finish, stream, publisher, or IRQ marker'
fi
if grep -a -E -q "${FAMILY} (PASS|TOPOLOGY|FINISH|STREAM|PUBLISH|PUBLISHER)([ =]|$)|${FAMILY} .* (finish|stream|publisher)=" "$QEMU_LOG"; then
  fail 'IRQ-overlay family emitted a forbidden formal, finish, stream, or publisher marker'
fi
foreign_families=$(grep -a -o -E 'WASM_[A-Z0-9_]+' "$QEMU_LOG" \
  | sort -u \
  | grep -E -v "^(${FAMILY}|${PHASE_FAMILY}|${CORE_FAMILY}|${REQUEST_FAMILY})$" || true)
[ -z "$foreign_families" ] \
  || fail "diagnostic image emitted foreign WASM families: $foreign_families"

python3 -B scripts/c84-ssh-managed-child-irq-overlay-peer.py \
  --verify-log-only --qemu-log "$QEMU_LOG" >&2 \
  || fail 'frozen IRQ/phase/Core/request transcript verification failed'

RESULT_REPORTED=1
sed -n '1,8p' "$PEER_LOG"
echo 'PASS qemu-c84-ssh-managed-child-irq-overlay-test: real epoch-1 parent Host and child Core causal self-SSIP pairs, epochs 2-4 active-pair isolation, per-epoch inactive terminal SSIP with ACTIVE_EPOCH=0, cancel/ack/Ready reuse, exact six-marker IRQ family, and unchanged phase/19-Core/8-request transcripts passed on one QEMU hart (integration evidence only)'
