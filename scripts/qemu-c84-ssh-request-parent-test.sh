#!/bin/sh
# C8.4 authenticated OpenSSH request-parent ownership and slot-reuse gate.
set -eu
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$RESULT_REPORTED" -eq 0 ]; then
    echo 'FAIL qemu-c84-ssh-request-parent-test: test aborted unexpectedly' >&2
  fi
  exit "$status"
}
...
echo 'qemu-c84-ssh-request-parent-test: building isolated single-hart OpenSSH image'
...
if ! python3 -B scripts/c84-ssh-request-parent-peer.py \
  --host localhost \
  --port "$SSH_HOST_PORT" \
  ...
  >"$PEER_LOG" 2>&1; then
  fail 'real OpenSSH request-parent sequence failed'
fi

kill -0 "$QEMU_PID" 2>/dev/null || fail 'QEMU exited during the OpenSSH sequence'
sleep 0.2
if grep -a -F -q "$FAMILY FAIL" "$QEMU_LOG"; then
  fail 'guest reported a request-parent failure after the final response'
fi
...
RESULT_REPORTED=1
sed -n '1,6p' "$PEER_LOG"
echo 'PASS qemu-c84-ssh-request-parent-test: real OpenSSH exact target admission, two-request reuse, active-Drop cleanup, post-Drop reuse, and response-boundary closure passed on one QEMU hart (integration evidence only)'
