...
pub enum ServEvent<'g, 'a> {
    ...
    /// Legacy first-authentication event.
    ///
    /// The VibeOS public-key-only profile never emits this event and its
    /// `allow()` method always fails closed.
    FirstAuth(ServFirstAuth<'g, 'a>),
    /// Legacy password-authentication event.
    ///
    /// The VibeOS public-key-only profile never emits this event and its
    /// `allow()` method always fails closed.
...
impl<'g, 'a> ServHostkeys<'g, 'a> {
    ...
    /// Complete key exchange with an external Ed25519 signer.
    ///
    /// The signer is borrowed only for this call. Sunset retains neither the
    /// signer nor any private key material.
    pub fn hostkey_ed25519(self, signer: &mut dyn Ed25519HostSigner) -> Result<()> {
        self.runner.resume_servhostkey_ed25519(signer)
    }
}
...
impl<'g, 'a> ServPasswordAuth<'g, 'a> {
    ...
    /// Password login is forbidden by the VibeOS profile.
    ...
    /// Keep password authentication disabled for subsequent attempts.
    ///
    /// Passing `true` fails with [`Error::BadUsage`].
    pub fn enable_password_auth(&mut self, enabled: bool) -> Result<()> {
        reject_password_enable(enabled)?;
        let (_, pubkey) = self.runner.auth_methods()?;
        self.runner.set_auth_methods(false, pubkey)
    }
    ...
    /// Configure public-key authentication for subsequent attempts.
    ///
    /// Passing `password = true` fails with [`Error::BadUsage`].
    pub fn set_auth_methods(&mut self, password: bool, pubkey: bool) -> Result<()> {
        reject_password_enable(password)?;
        self.runner.set_auth_methods(false, pubkey)
    }
...
impl<'g, 'a> ServFirstAuth<'g, 'a> {
    ...
    /// Unauthenticated login is forbidden by the VibeOS profile.
    pub fn allow(mut self) -> Result<()> {
        self.done = true;
        error::BadUsage.fail()
    }
    ...
    /// Keep password authentication disabled for this session.
    ///
    /// Passing `true` fails with [`Error::BadUsage`].
    pub fn enable_password_auth(&mut self, enabled: bool) -> Result<()> {
        reject_password_enable(enabled)?;
    ...
    /// Configure public-key authentication for this session.
    ///
    /// Passing `password = true` fails with [`Error::BadUsage`].
    pub fn set_auth_methods(&mut self, password: bool, pubkey: bool) -> Result<()> {
        reject_password_enable(password)?;
...
fn reject_password_enable(enabled: bool) -> Result<()> {
    if enabled { error::BadUsage.fail() } else { Ok(()) }
}
...
#[expect(
    dead_code,
    reason = "legacy event variants remain source-compatible but fail closed"
)]
