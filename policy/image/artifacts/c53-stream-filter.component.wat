(component
  ;; The memory provider must be instantiated before the guest because the
  ;; imported `read` lowering needs a guest memory and realloc. Keep exactly
  ;; this provider + guest topology: image policy admits two runtime Core
  ;; instances, never an adjacent spare instance.
  (core module $memory-provider
    (memory (export "memory") 1 1)
    ;; 0 reusable base; 4 allocs; 8 shrinks; 12 frees; 16 mismatch; 20 live pointer;
    ;; 24 live size; 28 last old size; 32 last new size; 36 run count.
    (data (i32.const 0) "\00\10\00\00")

    ;; A deliberately strict single-live-allocation allocator. The runtime
    ;; first reserves its bounded maximum for `read`, shrinks it to the exact
    ;; returned byte count, and the guest later frees with that exact old size.
    ;; Any stale pointer, imprecise old size, growth, or wrong alignment traps.
  ...
  (core module $guest
    ...
    (func (export "run") (param $input i32) (param $output i32)
      ...
      block $eof
        loop $pump
          ;; Canonical `read` writes its list pair to the retptr at 64.
          local.get $input
          i32.const 64
          call $read
          i32.const 64
          ...
          ;; Transform exactly the returned chunk in place. XOR 0x20 is the
          ;; frozen case-filter behavior for every byte, not an ASCII parser.
          i32.const 0
          local.set $index
          block $transformed
            loop $transform
          ...
          ;; The allocator rejects any old-size other than the exact length
          ;; published by the runtime's max-to-exact shrink transaction.
          local.get $pointer
          local.get $length
          i32.const 1
          i32.const 0
      ...
      ;; Only EOF reaches these exact enum-zero (`normal`) closes.
      local.get $input
      i32.const 0
      call $close-reader
      local.get $output
