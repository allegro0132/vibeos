#!/bin/sh
# C6.4 two-node supervised Component resource-route acceptance.
set -eu
...
C64_TIMEOUT=${C64_TIMEOUT:-60}
PASS_MARKER='WASM_C64_RESOURCE_ROUTE PASS nodes=2 own=1 borrow=1 guest_calls=0 fuel_consumed=0 provider_peak=1 provider_live=0 consumer_peak=1 consumer_live=0 target_revoked=1 source_revoked=0 target_first=1 runtime_unavailable=2 registry_occupied=0 registry_header_mismatches=0'
FAIL_MARKER='WASM_C64_RESOURCE_ROUTE FAIL'
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$RESULT_REPORTED" -eq 0 ]; then
    echo 'FAIL qemu-c64-resource-route-test: test aborted unexpectedly' >&2
  fi
  exit "$status"
}
...
fail() {
  RESULT_REPORTED=1
  echo "FAIL qemu-c64-resource-route-test: $*" >&2
  if [ -s "$QEMU_LOG" ]; then
...
case "$C64_TIMEOUT" in
  ''|*[!0-9]*|0) fail 'C64_TIMEOUT must be a positive integer' ;;
esac
...
(cd firmware/qemu-virt && \
  ...
  rustup run "$toolchain" cargo build --release \
    --features wasm-c64-resource-route-acceptance) >&2
...
QEMU_PID=$!
(sleep "$C64_TIMEOUT"; kill "$QEMU_PID" 2>/dev/null || true) &
KILLER_PID=$!

remaining=$((C64_TIMEOUT * 10))
while [ "$remaining" -gt 0 ]; do
  if grep -a -F -q "$FAIL_MARKER" "$QEMU_LOG"; then
    fail 'guest reported resource-route lifecycle failure'
  fi
  ...
  if [ "$pass_count" -eq 1 ]; then
    sleep 0.1
    if grep -a -F -q "$FAIL_MARKER" "$QEMU_LOG"; then
      fail 'guest reported resource-route failure after PASS'
    fi
    if grep -a -E -q '\[!\] (fatal|panic)|panicked at' "$QEMU_LOG"; then
    ...
    RESULT_REPORTED=1
    echo 'PASS qemu-c64-resource-route-test: exact supervised edge lifecycle completed'
    exit 0
  fi
...
fail 'timed out waiting for the exact C6.4 marker'
