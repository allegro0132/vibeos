...
def run(kernel, output, dtb=None, expect_rejection=False):
    ...
    with (output / 'serial.log').open('xb') as log, tempfile.TemporaryDirectory(prefix='vibeos-dtb-') as work:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                failure = re.search(rb'BOOT_DTB_CPUS FAIL: ([^\r\n]+)[\r\n]', data)
                if failure:
...
if __name__ == '__main__':
    ...
    print(json.dumps(run(args.kernel, args.output, args.dtb, args.expect_rejection), indent=2))
