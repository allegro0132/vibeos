...
def main():
    ...
    profile.add_argument('--python',action='store_true',help='use the Python image upload ceiling and large-module startup timeout')
    profile.add_argument('--esbuild',action='store_true',help='use the esbuild image upload ceiling and large-module startup timeout')
    ...
    large_module = args.python or args.esbuild
    if large_module:
    ...
    if args.action=='upload':
        ...
        limit=24*1024*1024 if args.esbuild else (16*1024*1024 if args.python else 512*1024)
    ...
    if args.action=='upload':
        result=subprocess.run(command,input=data,timeout=600 if large_module else 180)
        if result.returncode==0:print(f'Uploaded {name} ({len(data)} bytes, SHA-256 {hashlib.sha256(data).hexdigest()})')
        return result.returncode
    return subprocess.run(command,timeout=600 if large_module else None).returncode
