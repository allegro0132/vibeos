...
def analyze(text):
    ...
    for line in text.splitlines():
        ...
        values = arrays(line)
        required = {'ticks': 8, 'wait': 8, 'calls': 8}
        if match:
    ...
    work = [sum(b['ticks'][i] for b in buckets.values()) for i in range(8)]
    wait = [sum(b['wait'][i] for b in buckets.values()) for i in range(8)]
    for key, total in [('ticks', work), ('wait', wait)]:
        if total != [sum(h[key][i] for h in harts.values()) for i in range(8)]:
    ...
    for match in re.finditer(r'NPROF_LOCK h=(\d+) address=(0x[0-9a-f]+) ([^\r\n]+)', text):
        ...
        for name in ['wait', 'calls']:
            if len(values.get(name, [])) != 8 or any(type(n) is not int or n < 0 for n in values[name]):
    ...
    if locks:
        for h, totals in harts.items():
            if totals['wait'] != [sum(r['wait'][i] for r in locks if r['hart'] == h) for i in range(8)]:
    ...
    return dict(start=start, end=end, width=width, phases=phases, harts=harts,
                queues=queues, dma_observations=dma, buckets=buckets, locks=locks,
                limitations=['Elapsed 4 MHz timer ticks on Mars, not CPU cycles.',
