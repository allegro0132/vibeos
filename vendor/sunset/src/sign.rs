...
/// External Ed25519 host-key signer.
///
/// Sunset copies only the public key. The private key remains in the
/// implementation (for example, behind an OS capability). Each call receives
/// exactly the 32-byte SSH exchange hash `H` and returns its raw 64-byte
/// Ed25519 signature.
pub trait Ed25519HostSigner {
    fn public_key(&self) -> Result<[u8; 32]>;

    fn sign_exchange_hash(&mut self, exchange_hash: &[u8; 32]) -> Result<[u8; 64]>;
}
...
impl SignKey {
    /// Generate a local signing key with caller-provided randomness.
    ///
    /// SSH session processing never calls this method. Applications that keep
    /// host secrets outside Sunset can instead use [`Ed25519HostSigner`].
    pub fn generate<R: rand_core::CryptoRng>(
        ty: KeyType,
        bits: Option<usize>,
        random: &mut R,
    ) -> Result<Self> {
        match ty {
            KeyType::Ed25519 => {
                ...
                let k = dalek::SigningKey::generate(random);
                Ok(Self::Ed25519(k))
            }
            ...
            KeyType::RSA => {
                ...
                let k = rsa::RsaPrivateKey::new(random, bits).map_err(|e| {
            ...
            KeyType::ECDSA256 => {
                ...
                let k = p256::SecretKey::generate_from_rng(random);
...
pub mod tests {
    ...
    #[test]
    fn signkey_enc_dec() {
        let mut random = TestRandom::new(0x41);
        let tys = [
            KeyType::Ed25519,
            #[cfg(feature = "rsa")]
            KeyType::RSA,
        ...
        for _ in 0..20 {
            for ty in tys {
                let k = SignKey::generate(ty, None, &mut random).unwrap();
                let mut v = vec![];
