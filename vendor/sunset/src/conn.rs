...
pub(crate) struct Conn<CS: CliServ> {
    ...
    /// The peer negotiated strict handling for the initial key exchange.
    /// Kept separately from `Kex` so it remains set after receiving NEWKEYS
    /// until the connection state advances out of `FirstKex`.
    strict_initial_kex: bool,
...
enum ConnState {
    ...
    /// After auth success
    Authed,
    /// The peer sent SSH_MSG_DISCONNECT. No further protocol progress is valid.
    Defunct,
}
...
impl<CS: CliServ> Conn<CS> {
    ...
    pub(crate) fn progress(
        ...
        if matches!(self.state, ConnState::Defunct) {
        ...
        match self.state {
            ...
            ConnState::FirstKex => {
                if self.sess_id.is_some() && self.kex.is_idle() {
                    disp.event = DispatchEvent::Progressed;
    ...
    pub(crate) fn handle_payload(
        ...
        match self.packet(payload) {
            ...
            Err(Error::UnknownPacket { number }) => {
                if self.is_strict_initial_kex() {
                    debug!("Unknown packet type {number} during strict initial KEX");
    ...
    fn check_packet(&self, p: &Packet) -> Result<()> {
        ...
        // A disconnect is terminal rather than part of the KEX transcript. It
        // is safe and useful to consume it even while strict KEX is active.
        if matches!(p, Packet::Disconnect(_)) {
            return Ok(());
        }

        let r = if self.is_strict_initial_kex() {
    ...
    fn is_first_kex(&self) -> bool {
        matches!(
            self.state,
            ConnState::SendIdent | ConnState::ReceiveIdent | ConnState::FirstKex
        )
    }

    fn is_strict_initial_kex(&self) -> bool {
        self.is_first_kex() && self.strict_initial_kex
    }
    ...
    pub fn dispatch_packet(
        ...
        match packet {
            Packet::KexInit(k) => {
                ...
                if first_kex && self.kex.is_strict() {
                    self.strict_initial_kex = true;
                }
            }
            Packet::KexDHInit(_p) => {
...
impl Conn<Server> {
    ...
    pub(crate) fn resume_servhostkey_ed25519(
        &mut self,
        payload: &[u8],
        s: &mut TrafSend,
        ...
        let packet = self.packet(payload)?;
        if let Packet::KexDHInit(p) = packet {
            self.kex.resume_kexdhinit_ed25519(
                &p,
                self.is_first_kex(),
                signer,
                &mut self.sess_id,
...
mod tests {
    ...
    use crate::packets::{Ignore, KexCookie};
    use crate::random::tests::TestRandom;
    use crate::sshwire::BinString;
    use crate::traffic::TrafOut;

    fn strict_initial_conn() -> Conn<Server> {
        let mut conn = Conn::<Server>::new();
        conn.state = ConnState::FirstKex;
        conn.strict_initial_kex = true;
        conn
    }

    #[test]
    fn strict_initial_unknown_packet_fails_without_unimplemented() {
        let mut conn = strict_initial_conn();
        let mut output = [0u8; 512];
        let mut traf_out = TrafOut::new(&mut output);
        let mut keys = KeyState::new_cleartext();
        let mut random = TestRandom::new(0x61);

        let result = {
            let mut sender = traf_out.sender(&mut keys, &mut random);
            conn.handle_payload(&[42], 7, &mut sender)
        };
        assert!(matches!(result, Err(Error::SSHProto { .. })));
        assert!(traf_out.output_buf().is_empty());
    }

    #[test]
    fn strict_initial_ignore_fails_without_output() {
        let mut conn = strict_initial_conn();
        let mut payload = [0u8; 64];
        let len = sshwire::write_ssh(
            &mut payload,
            &Packet::Ignore(Ignore { data: BinString(b"ignored") }),
        )
        .unwrap();
        ...
        let mut traf_out = TrafOut::new(&mut output);
        let mut keys = KeyState::new_cleartext();
        let mut random = TestRandom::new(0x62);
        let result = {
            let mut sender = traf_out.sender(&mut keys, &mut random);
            conn.handle_payload(&payload[..len], 7, &mut sender)
        };
        assert!(matches!(result, Err(Error::SSHProto { .. })));
        assert!(traf_out.output_buf().is_empty());
    }
    ...
    #[test]
    fn unknown_packet_outside_strict_kex_gets_unimplemented() {
        let mut conn = Conn::<Server>::new();
        conn.state = ConnState::PreAuth;

        let mut output = [0u8; 512];
        let mut traf_out = TrafOut::new(&mut output);
        let mut keys = KeyState::new_cleartext();
        let mut random = TestRandom::new(0x63);
        let result = {
            let mut sender = traf_out.sender(&mut keys, &mut random);
            conn.handle_payload(&[42], 7, &mut sender)
        };
    ...
    #[test]
    fn first_kex_waits_for_peer_newkeys_before_preauth() {
        let mut conn = strict_initial_conn();
        conn.sess_id = Some(SessId::from_slice(&[0x64; 32]).unwrap());
        conn.kex = Kex::KexInit { our_cookie: KexCookie([0x65; 16]) };

        let mut output = [0u8; 512];
        let mut traf_out = TrafOut::new(&mut output);
        let mut keys = KeyState::new_cleartext();
        let mut random = TestRandom::new(0x66);
        {
            let mut sender = traf_out.sender(&mut keys, &mut random);
            conn.progress(&mut sender).unwrap();
        }
        assert!(matches!(conn.state, ConnState::FirstKex));
        ...
        assert!(matches!(progressed.event, DispatchEvent::Progressed));
        assert!(matches!(conn.state, ConnState::PreAuth));
    }

    #[test]
    fn disconnect_is_consumed_as_terminal_even_during_strict_kex() {
        let mut conn = strict_initial_conn();
        let packet =
            packets::Disconnect { reason: 11, desc: "done".into(), lang: "" };

        let mut output = [0u8; 512];
        let mut traf_out = TrafOut::new(&mut output);
        let mut keys = KeyState::new_cleartext();
        let mut random = TestRandom::new(0x67);
        let dispatched = {
            let mut sender = traf_out.sender(&mut keys, &mut random);
            conn.dispatch_packet(packet.into(), &mut sender).unwrap()
        };
        assert!(dispatched.disconnect);
        assert!(matches!(conn.state, ConnState::Defunct));

        let no_progress = {
            let mut sender = traf_out.sender(&mut keys, &mut random);
            conn.progress(&mut sender).unwrap()
        };
        assert!(no_progress.event.is_none());
    }
}
