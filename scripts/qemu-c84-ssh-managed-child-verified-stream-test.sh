#!/bin/sh
# C8.4 real SSH verified-summary/full-interval-stream/complete gate.
set -eu
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$RESULT_REPORTED" -eq 0 ]; then
    echo 'FAIL qemu-c84-ssh-managed-child-verified-stream-test: test aborted unexpectedly' >&2
  fi
  exit "$status"
}
...
fail() {
  RESULT_REPORTED=1
  echo "FAIL qemu-c84-ssh-managed-child-verified-stream-test: $*" >&2
...
# Run every predecessor gate first so the successor cannot hide a widened seam.
...
python3 -B scripts/verify-c84-ssh-managed-child-verified-stream.py --selftest --check-source >&2 \
  || fail 'static verified-stream verification failed'
...
echo 'qemu-c84-ssh-managed-child-verified-stream-test: building isolated single-hart OpenSSH image'
...
if ! python3 -B scripts/c84-ssh-managed-child-verified-stream-peer.py \
  --host localhost \
  --port "$SSH_HOST_PORT" \
  ...
  >"$PEER_LOG" 2>&1; then
  fail 'real OpenSSH verified-stream sequence failed'
fi

kill -0 "$QEMU_PID" 2>/dev/null || fail 'QEMU exited during the OpenSSH sequence'
stream_count_before=$(family_count "$FAMILY")
[ "$stream_count_before" -eq 4 ] \
  || fail "closed peer transcript did not contain exactly 4 verified-stream markers: $stream_count_before"
...
sleep 0.2
stream_count_after=$(family_count "$FAMILY")
...
[ "$stream_count_after" -eq "$stream_count_before" ] \
  || fail 'late verified-stream marker arrived after peer closure'
[ "$finish_count_after" -eq "$finish_count_before" ] \
  || fail 'late finish marker arrived after peer closure'
...
# Stop the UART producer before the final exact parse so no partial late record
# can be hidden or admitted.
stop_qemu
stream_count_frozen=$(family_count "$FAMILY")
...
[ "$stream_count_frozen" -eq "$stream_count_after" ] \
  || fail 'verified-stream marker arrived while freezing QEMU'
[ "$finish_count_frozen" -eq "$finish_count_after" ] \
  || fail 'finish marker arrived while freezing QEMU'
...
if grep -a -E -q "(${FAMILY}|${FINISH_FAMILY}|${IRQ_FAMILY}|${PHASE_FAMILY}|${CORE_FAMILY}|${REQUEST_FAMILY}) (PASS|TOPOLOGY|SUMMARY|INTERVAL|COMPLETE|STREAM|PUBLISH|PUBLISHER|SCHEMA|COLLECTOR)([ =]|$)" "$QEMU_LOG"; then
  fail 'diagnostic image emitted forbidden stream, publisher, schema, collector, or formal telemetry'
fi
...
python3 -B scripts/c84-ssh-managed-child-verified-stream-peer.py \
  --verify-log-only --qemu-log "$QEMU_LOG" >&2 \
  || fail 'frozen verified/finish/IRQ/phase/Core/request transcript verification failed'

RESULT_REPORTED=1
sed -n '1,8p' "$PEER_LOG"
echo 'PASS qemu-c84-ssh-managed-child-verified-stream-test: epochs 1/2/4 verified positive bounded summaries, streamed exact contiguous phase-rescanned intervals through dynamic count=emitted=cursor, completed to clean Ready, preserved epoch-3 LeaseCancelled Drop, and kept every predecessor exact on one QEMU hart (integration evidence only)'
