...
// VibeOS exposes a deliberately small server profile. Marker names live in the
// KEX name-list, but aren't negotiable algorithms themselves.
const strict_server_kex: &[&str] = &[SSH_NAME_CURVE25519, SSH_NAME_STRICT_KEX_S];
const strict_server_hostsig: &[&str] = &[SSH_NAME_ED25519];
const strict_server_cipher: &[&str] = &[SSH_NAME_CHAPOLY];
const strict_server_mac: &[&str] = &[SSH_NAME_HMAC_SHA256];
const strict_server_comp: &[&str] = &[SSH_NAME_NONE];
...
impl AlgoConfig {
    ...
    pub fn new(is_client: bool) -> Self {
        if !is_client {
            return AlgoConfig {
                kexs: strict_server_kex.try_into().unwrap(),
                hostsig: strict_server_hostsig.try_into().unwrap(),
                ciphers: strict_server_cipher.try_into().unwrap(),
                macs: strict_server_mac.try_into().unwrap(),
                comps: strict_server_comp.try_into().unwrap(),
            };
        }
...
impl KexCookie {
    pub fn generate(random: &mut dyn RandomSource) -> Result<Self> {
...
impl<CS: CliServ> Kex<CS> {
    ...
    /// Whether no key exchange is currently in progress.
    ...
    pub fn progress(&mut self, conf: &AlgoConfig, s: &mut TrafSend) -> Result<()> {
        ...
        match self {
            ...
            Kex::StartKexInit => {
                if s.is_drained() {
                    let our_cookie = KexCookie::generate(s.random())?;
    ...
    pub fn handle_kexinit(
        ...
        // The VibeOS server profile requires OpenSSH strict KEX. Validate the
        // peer's marker and packet ordering before generating a cookie or an
        // ephemeral X25519 secret, so rejected handshakes consume no entropy.
        if !CS::is_client() && first_kex {
            check_strict_server_initial(&remote_kexinit, s.recv_seq())?;
        }

        let our_cookie = match self {
            Kex::KexInit { our_cookie } => our_cookie.clone(),
            Kex::Idle | Kex::StartKexInit => KexCookie::generate(s.random())?,
            _ => {
        ...
        let mut algos =
            Self::algo_negotiation(&remote_kexinit, algo_conf, s.random())?;
        debug!("{algos}");
    ...
    fn algo_negotiation(
        ...
        let kex = SharedSecret::from_name(kex_method, random)?;
...
fn check_strict_server_initial(
    remote_kexinit: &packets::KexInit,
    recv_seq: u32,
) -> Result<()> {
    if !remote_kexinit.kex.has_algo(SSH_NAME_STRICT_KEX_C)? {
        debug!("client did not offer the required strict KEX marker");
        return Err(Error::AlgoNoMatch { algo: "strict kex" });
    }
    if recv_seq != 1 {
        debug!("strict KEXINIT wasn't the first binary packet");
        return error::PacketWrong.fail();
    }
    if remote_kexinit.first_follows {
        debug!("first_kex_packet_follows is disabled by the strict server profile");
        return error::PacketWrong.fail();
    }
    Ok(())
}
...
impl Kex<Server> {
    ...
    pub fn resume_kexdhinit_ed25519(
        &mut self,
        p: &packets::KexDHInit,
        first_kex: bool,
        signer: &mut dyn Ed25519HostSigner,
        sess_id: &mut Option<SessId>,
        s: &mut TrafSend,
    ) -> Result<()> {
        if let Kex::KexDH { mut algos, kex_hash } = self.take() {
            ...
            let output = SharedSecret::handle_kexdhinit_ed25519(
                &mut algos, kex_hash, signer, p, s,
            )?;
            self.send_newkeys(output, algos, sess_id, s)?;
...
impl SharedSecret {
    pub fn from_name(
        ...
        match name {
            SSH_NAME_CURVE25519 | SSH_NAME_CURVE25519_LIBSSH => {
                Ok(SharedSecret::KexCurve25519(KexCurve25519::new(random)?))
            }
            #[cfg(feature = "mlkem")]
            SSH_NAME_MLKEM_X25519 => {
                Ok(SharedSecret::KexMlkemX25519(KexMlkemX25519::new(random)?))
            }
            _ => Error::bug(),
        }
    }
    ...
    fn send_kexdhinit(&mut self, s: &mut TrafSend) -> Result<()> {
        ...
        let q_c = match self {
            ...
            Self::KexMlkemX25519(k) => {
                mlkem_bytes = k.init_pubkey_arr_client(s.random())?;
                &mlkem_bytes
            }
        };
    ...
    fn handle_kexdhinit(
        ...
        let (kex_out, pubkey) = match &mut algos.kex {
            ...
            SharedSecret::KexMlkemX25519(k) => {
                let ko;
                (ko, mlkem_bytes) =
                    k.secret_encap_server(p.q_c.0, kex_hash, s.random())?;
    ...
    /// Server key exchange using an Ed25519 key that never enters Sunset.
    fn handle_kexdhinit_ed25519(
        algos: &mut Algos<Server>,
        mut kex_hash: KexHash,
        signer: &mut dyn Ed25519HostSigner,
        p: &packets::KexDHInit,
        ...
        if !matches!(algos.hostsig, SigType::Ed25519) {
            return Err(Error::AlgoNoMatch { algo: "external hostkey" });
        }
        ...
        let hostkey =
            PubKey::Ed25519(packets::Ed25519PubKey { key: Blob(public_key) });
        kex_hash.hash_hostkey(&hostkey)?;
        ...
        let h: &[u8; 32] =
            kex_out.h.as_slice().try_into().map_err(|_| Error::BadKex)?;
        let signature = signer.sign_exchange_hash(h)?;

        // A faulty signer must not put an invalid host signature on the wire.
        let signature_value = ed25519_dalek::Signature::from_bytes(&signature);
        verifying_key
            .verify_strict(h, &signature_value)
            .map_err(|_| Error::BadSig)?;

        let signature = OwnedSig::Ed25519(signature);
        Self::send_kexdhreply_parts(&kex_out, pubkey, hostkey, &signature, s)?;
        Ok(kex_out)
    }
    ...
    pub fn send_kexdhreply(
        ...
        Self::send_kexdhreply_parts(ko, kex_pub, hostkey.pubkey(), &sig, s)
    }

    fn send_kexdhreply_parts(
        ko: &KexOutput,
        kex_pub: &[u8],
        hostkey: PubKey<'_>,
        sig: &OwnedSig,
        ...
        let k_s = Blob(hostkey);
        trace!("send kexreply h {:02x?}", ko.h.as_slice());
...
impl KexCurve25519 {
    fn new(random: &mut dyn RandomSource) -> Result<Self> {
        let mut seeded = random::FixedRandom32::new(random)?;
        let ours = x25519_dalek::EphemeralSecret::random_from_rng(&mut seeded);
        seeded.consumed_exactly()?;
...
impl KexMlkemX25519 {
    ...
    fn new(random: &mut dyn RandomSource) -> Result<Self> {
        Ok(Self { ecdh: KexCurve25519::new(random)?, mlkem_seed: None })
    }
    ...
    fn init_pubkey_arr_client(
        &mut self,
        random: &mut dyn RandomSource,
    ) -> Result<[u8; Self::PUBLICKEY_SIZE]> {
...
mod tests {
    ...
    fn client_kexinit(
        ...
        ciphers: &'static str,
        first_follows: bool,
    ) -> packets::KexInit<'static> {
        packets::KexInit {
            cookie: KexCookie([0x42; 16]),
            kex: kexs.try_into().unwrap(),
            hostsig: "rsa-sha2-256,ssh-ed25519".try_into().unwrap(),
            cipher_c2s: ciphers.try_into().unwrap(),
            cipher_s2c: ciphers.try_into().unwrap(),
    ...
    #[test]
    fn strict_server_advertises_exact_algorithm_profile() {
        let conf = AlgoConfig::new(false);
        assert_eq!(conf.kexs.0.as_slice(), strict_server_kex);
        assert_eq!(conf.hostsig.0.as_slice(), strict_server_hostsig);
        assert_eq!(conf.ciphers.0.as_slice(), strict_server_cipher);
        assert_eq!(conf.macs.0.as_slice(), strict_server_mac);
        assert_eq!(conf.comps.0.as_slice(), strict_server_comp);
    }

    #[test]
    fn strict_server_requires_marker_first_packet_and_no_guess() {
        let valid = client_kexinit(
            "sntrup761x25519-sha512,curve25519-sha256,kex-strict-c-v00@openssh.com,curve25519-sha256@libssh.org",
            "aes256-ctr,chacha20-poly1305@openssh.com",
            false,
        );
        check_strict_server_initial(&valid, 1).unwrap();

        let missing_marker =
            client_kexinit(SSH_NAME_CURVE25519, SSH_NAME_CHAPOLY, false);
        assert!(matches!(
            check_strict_server_initial(&missing_marker, 1),
            Err(Error::AlgoNoMatch { algo: "strict kex" })
        ));
        assert!(matches!(
            check_strict_server_initial(&valid, 2),
            Err(Error::PacketWrong { .. })
        ));

        let guessed = client_kexinit(
            "curve25519-sha256,kex-strict-c-v00@openssh.com",
            SSH_NAME_CHAPOLY,
            true,
        );
        assert!(matches!(
            check_strict_server_initial(&guessed, 1),
            Err(Error::PacketWrong { .. })
        ));
    }

    #[test]
    fn strict_server_ignores_peer_extras_but_never_selects_them() {
        let conf = AlgoConfig::new(false);
        let mut random = TestRandom::new(0x50);
        let offered = client_kexinit(
            "sntrup761x25519-sha512,curve25519-sha256,kex-strict-c-v00@openssh.com,curve25519-sha256@libssh.org",
            "aes256-ctr,chacha20-poly1305@openssh.com",
            false,
        );
        let algos =
            Kex::<Server>::algo_negotiation(&offered, &conf, &mut random).unwrap();
        assert_eq!(algos.kex.to_string(), SSH_NAME_CURVE25519);
        assert_eq!(algos.hostsig.algorithm_name(), SSH_NAME_ED25519);
        assert_eq!(algos.cipher_enc.to_string(), SSH_NAME_CHAPOLY);
        assert_eq!(algos.cipher_dec.to_string(), SSH_NAME_CHAPOLY);
        assert!(algos.strict_kex);

        let alias_only = client_kexinit(
            "curve25519-sha256@libssh.org,kex-strict-c-v00@openssh.com",
            SSH_NAME_CHAPOLY,
            false,
        );
        assert!(matches!(
            Kex::<Server>::algo_negotiation(&alias_only, &conf, &mut random),
            Err(Error::AlgoNoMatch { algo: "kex" })
        ));

        let aes_only = client_kexinit(
            "curve25519-sha256,kex-strict-c-v00@openssh.com",
            SSH_NAME_AES256_CTR,
            false,
        );
        assert!(matches!(
            Kex::<Server>::algo_negotiation(&aes_only, &conf, &mut random),
            Err(Error::AlgoNoMatch { algo: "encryption" })
        ));
    }

    #[test]
    fn test_name_match() {
        let mut random = TestRandom::new(0x51);
        // check that the from_name() functions are complete
        for k in kex::fixed_options_kex.iter() {
            kex::SharedSecret::from_name(k, &mut random).unwrap();
        }
    ...
    #[should_panic]
    fn test_unknown_kex() {
        let mut random = TestRandom::new(0x52);
        kex::SharedSecret::from_name("bad", &mut random).unwrap();
    }
    #[test]
    #[should_panic]
    fn test_unknown_sig() {
    ...
    impl TrafCatcher {
        ...
        fn sender<'f>(&'f mut self) -> traffic::TrafSend<'f, 'static> {
            self.traf_out.sender(&mut self.keys, &mut self.random)
        }
    ...
    #[test]
    fn test_each_kex() {
        test_kex_allow(SSH_NAME_CURVE25519, false)
    }

    #[test]
    fn external_ed25519_signer_receives_exact_exchange_hash() {
        test_kex_allow(SSH_NAME_CURVE25519, true)
    }

    struct RecordingSigner {
        signing_key: ed25519_dalek::SigningKey,
        exchange_hash: Option<[u8; 32]>,
    }

    impl RecordingSigner {
        fn new() -> Self {
            Self {
                signing_key: ed25519_dalek::SigningKey::from_bytes(&[0x71; 32]),
                exchange_hash: None,
            }
        }
    }

    impl Ed25519HostSigner for RecordingSigner {
        ...
        fn sign_exchange_hash(
            ...
            Ok(self.signing_key.sign(exchange_hash).to_bytes())
        }
    }

    fn test_kex_allow(chosen_kex: &'static str, external_signer: bool) {
        // #![allow(unused)]
        init_test_log();
        ...
        cli_conf.kexs.0.push(SSH_NAME_STRICT_KEX_C).unwrap();
        ...
        let mut key_random = TestRandom::new(0x54);
        let keys = [crate::SignKey::generate(
        ...
        if external_signer {
            let exchange_hash = signer.exchange_hash.expect("signer was called");
            let expected = signer.signing_key.sign(&exchange_hash).to_bytes();
            match &serv_dhrep.sig.0 {
                Signature::Ed25519(sig) => assert_eq!(sig.sig.0, expected),
                _ => panic!("external signer produced a non-Ed25519 signature"),
            }
            match &serv_dhrep.k_s.0 {
                PubKey::Ed25519(key) => assert_eq!(
                    key.key.0,
                    signer.signing_key.verifying_key().to_bytes()
                ),
                _ => panic!("external signer produced a non-Ed25519 hostkey"),
            }
        }
    ...
    fn roundtrip(payload: &[u8], enc: &mut KeyState, dec: &mut KeyState) {
        ...
        let mut random = TestRandom::new(0x55);
        let l = enc.encrypt(payload.len(), &mut b, &mut random).unwrap();
