...
impl TryFrom<f64> for AnyConst32 {
    ...
    fn try_from(value: f64) -> Result<Self, Self::Error> {
        let truncated_bits = wasmi_core::softfloat::f32_demote_f64_bits(value.to_bits());
        if value.to_bits() != wasmi_core::softfloat::f64_promote_f32_bits(truncated_bits) {
            return Err(OutOfBoundsConst);
        }
        Ok(Self::from(f32::from_bits(truncated_bits)))
    }
}
...
impl From<AnyConst32> for f64 {
    fn from(value: AnyConst32) -> Self {
        Self::from_bits(wasmi_core::softfloat::f64_promote_f32_bits(u32::from(
            value,
        )))
    }
}
