...
impl KeyState {
    ...
    pub fn encrypt(
        ...
        let e = self.enc.encrypt(payload_len, buf, self.seq_encrypt.0, random);
...
mod tests {
    ...
    fn algo_combos() -> impl Iterator<Item = Option<kex::Algos<Client>>> {
        ...
        let mut random = TestRandom::new(0x32);
        COMBOS
            .iter()
            .map(move |(ce, ie, cd, id)| {
                Some(kex::Algos {
                    kex: kex::SharedSecret::from_name(
                        SSH_NAME_CURVE25519,
                        &mut random,
                    )
                    .unwrap(),
