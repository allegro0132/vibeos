...
| `kernel::world::Component::lifecycle` (vtop) | Cold operator/supervisor restart and removal; never acquired by IRQs or held across await | Retain: exact-instance check, cancellation or fresh-grant publication, and retirement are serialized. Recoverable because template creation may allocate/fault. Acquired before short registry/instance/heap/CSpace operations; registry lookup releases its guard before acquiring this lock. |
